...
- `MiningScripts/Inventory.lua`: inventory helper. `unloadBehind()` turns around, drops all non-empty slots into the chest behind the turtle, and turns back to preserve facing. `dropJunk()` drops common filler blocks such as cobblestone, cobbled deepslate, dirt, gravel, tuff, granite, diorite, and andesite while keeping ores, coal, raw metals, gems, tools, fuel, and unknown items.
...
- `Mine:run()` currently mines a wide straight tunnel in facing `1` / positive X from the `Y=16` mine start. Each step mines up plus left/right side blocks, then moves forward. It intentionally does not dig down during strip mining. Before deciding inventory is full, it calls `Inventory:dropJunk()` so cobblestone/deepslate filler does not force early return. It resumes by walking `mineDistance` blocks from the mine start before continuing.
